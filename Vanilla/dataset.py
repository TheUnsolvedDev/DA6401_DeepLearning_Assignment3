...
def data(language: str = 'bn', type: str = 'train') -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    path = "./dakshina_dataset/{}/lexicons/{}.translit.sampled.{}.tsv".format(
        language, language, type)
    df = pd.read_csv(path, header=None, sep='\t')
    return np.array(df[1]), np.array(df[0])
...
def get_dataloader(batch_size: int = config.BATCH_SIZE) -> Tuple[Corpus, Corpus, torch.utils.data.DataLoader, torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    """
    ...
    Returns:
        ...
        The first DataLoader is for training, the second is for validation,
        and the third is for testing.
    """
    input_lang, output_lang, pairs = set_words('bn','train')
    n = len(pairs)
    ...
    test_data = torch.utils.data.TensorDataset(torch.LongTensor(input_ids).to(device),
                                                torch.LongTensor(target_ids).to(device))
...
if __name__ == '__main__':
    ...
    print(sorted(output_lang.word_to_index, key=lambda x: output_lang.word_to_index[x]))
